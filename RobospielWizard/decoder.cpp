...
bool stuckLoopCatch=false;
bool loopCrashed=false;
...
int newHighest0=0,newHighest1=0,newHighest2=0,newHighest3=0,newHighest4=0,newHighest5=0,newHighest6=0,newHighest7=0,newHighest8=0,
newHighest10=0,newHighest11=0,newHighest12=0,newHighest13=0,newHighest14=0,newHighest15=0;
int newLowest0=127,newLowest1=127,newLowest2=127,newLowest3=127,newLowest4=127,newLowest5=127,newLowest6=127,newLowest7=127,newLowest8=127,
newLowest10=127,newLowest11=127,newLowest12=127,newLowest13=127,newLowest14=127,newLowest15=127;
...
void transmuteMenu();
void meldMenu();
void meld(int,int,int,int,int,int,int);
...
int main() {
    ...
    while (doneTracks<totalTracks) {
        ...
        if (!timeDivision) {
            ...
            */
            noteTime+=(unsigned long)((double)deltaTime*(600000.0/((double)tempo*(double)clicksPerBeat))+0.5);
            totalTime+=(unsigned long)((double)deltaTime*(600000.0/((double)tempo*(double)clicksPerBeat))+0.5);
            ...
            */
            ...
            noteTime+=(unsigned long)((double)deltaTime*(600000.0/(double)clicksPerSecond)+0.5);
            totalTime+=(unsigned long)((double)deltaTime*(600000.0/(double)clicksPerSecond)+0.5);
...
void fileOpener() {
    char tempName[120];
    string directoryName = "C:\\Users\\Kevin\\MIDI\\Songs\\";
    int dirSize=directoryName.size();
    int nameSize=0;
    do {
        cout << "Please tell me the name of the MIDI file. (include .mid)" << endl;
        cout << "C:\\Users\\Kevin\\MIDI\\Songs\\";
        cin.getline(tempName,120);
        for (int i=0;i<=dirSize;i++) {
            midiName[i]=directoryName[i];
        }
        for (int i=0;tempName[i]!='\0';i++) {
            nameSize++;
        }
        for (int i=dirSize;i<dirSize+nameSize;i++) {
            midiName[i]=tempName[i-dirSize];
        }
    ...
    do {
        ...
        cin.getline(tempName,120);
        for (int i=0;i<=dirSize;i++) {
            textName[i]=directoryName[i];
        }
        for (int i=0;tempName[i]!='\0';i++) {
            nameSize++;
        }
        for (int i=dirSize;i<dirSize+nameSize;i++) {
            textName[i]=tempName[i-dirSize];
        }
        textFile.open(textName);
...
unsigned long readVarLen() {
    ...
    while (true) {
        if ((tempVal/128)==0) {
            ...
            finalVal=finalVal*128;
            break;
        }
        outputters("entered time loop");
        finalVal=finalVal*128;
        finalVal+=(tempVal-128);
        ...
        if (255==tempVal)
        {
            ...
            stuckLoopCatch=false;
        }
        if (stuckLoopCount > 10)
        {
            loopCrashed=true;
            return 10000;
        }
...
void addTimeType0(short channel) {
	...
	switch (channel) {
		case 0x90:
            //dropped because it should be in the text only, or else double done when melding!
            /*if (delta0Time>vol-1) {
                delta0Time-=vol;
            } else {
                delta0Time=0;
...
void justify(int channel,char choice) {
    ...
    if (choice=='t'|choice=='T') {
        if (channel==0) {
            ...
            if(highest0-12*marker>0) {
                highest0-=(12*marker);
            } else {
                highest0=highest0%12;
            }
            if(lowest0-12*marker>0) {
                lowest0-=(12*marker);
            } else {
                lowest0=lowest0%12;
            }
            for (int i=0;i<channel0N.size();i++) {
                if(channel0N[i]-12*marker>0) {
                    channel0N[i]-=(12*marker);
                } else {
                    channel0N[i]=channel0N[i]%12;
                }
            }
        } else if (channel==1) {
            guide=highest1;
            ...
            if(highest1-12*marker>0) {
                highest1-=(12*marker);
            } else {
                highest1=highest1%12;
            }
            if(lowest1-12*marker>0) {
                lowest1-=(12*marker);
            } else {
                lowest1=lowest1%12;
            }
            for (int i=0;i<channel1N.size();i++) {
                if(channel1N[i]-12*marker>0) {
                    channel1N[i]-=(12*marker);
                } else {
                    channel1N[i]=channel1N[i]%12;
                }
            }
        } else if (channel==2) {
            guide=highest2;
            ...
            if(highest2-12*marker>0) {
                highest2-=(12*marker);
            } else {
                highest2=highest2%12;
            }
            if(lowest2-12*marker>0) {
                lowest2-=(12*marker);
            } else {
                lowest2=lowest2%12;
            }
            for (int i=0;i<channel2N.size();i++) {
                if(channel2N[i]-12*marker>0) {
                    channel2N[i]-=(12*marker);
                } else {
                    channel2N[i]=channel2N[i]%12;
                }
            }
        } else if (channel==3) {
            guide=highest3;
            ...
            if(highest3-12*marker>0) {
                highest3-=(12*marker);
            } else {
                highest3=highest3%12;
            }
            if(lowest3-12*marker>0) {
                lowest3-=(12*marker);
            } else {
                lowest3=lowest3%12;
            }
            for (int i=0;i<channel3N.size();i++) {
                if(channel3N[i]-12*marker>0) {
                    channel3N[i]-=(12*marker);
                } else {
                    channel3N[i]=channel3N[i]%12;
                }
            }
        } else if (channel==4) {
            guide=highest4;
            ...
            if(highest4-12*marker>0) {
                highest4-=(12*marker);
            } else {
                highest4=highest4%12;
            }
            if(lowest4-12*marker>0) {
                lowest4-=(12*marker);
            } else {
                lowest4=lowest4%12;
            }
            for (int i=0;i<channel4N.size();i++) {
                if(channel4N[i]-12*marker>0) {
                    channel4N[i]-=(12*marker);
                } else {
                    channel4N[i]=channel4N[i]%12;
                }
            }
        } else if (channel==5) {
            guide=highest5;
            ...
            if(highest5-12*marker>0) {
                highest5-=(12*marker);
            } else {
                highest5=highest5%12;
            }
            if(lowest5-12*marker>0) {
                lowest5-=(12*marker);
            } else {
                lowest5=lowest5%12;
            }
            for (int i=0;i<channel5N.size();i++) {
                if(channel5N[i]-12*marker>0) {
                    channel5N[i]-=(12*marker);
                } else {
                    channel5N[i]=channel5N[i]%12;
                }
            }
        } else if (channel==6) {
            guide=highest6;
            ...
            if(highest6-12*marker>0) {
                highest6-=(12*marker);
            } else {
                highest6=highest6%12;
            }
            if(lowest6-12*marker>0) {
                lowest6-=(12*marker);
            } else {
                lowest6=lowest6%12;
            }
            for (int i=0;i<channel6N.size();i++) {
                if(channel6N[i]-12*marker>0) {
                    channel6N[i]-=(12*marker);
                } else {
                    channel6N[i]=channel6N[i]%12;
                }
            }
        } else if (channel==7) {
            guide=highest7;
            ...
            if(highest7-12*marker>0) {
                highest7-=(12*marker);
            } else {
                highest7=highest7%12;
            }
            if(lowest7-12*marker>0) {
                lowest7-=(12*marker);
            } else {
                lowest7=lowest7%12;
            }
            for (int i=0;i<channel7N.size();i++) {
                if(channel7N[i]-12*marker>0) {
                    channel7N[i]-=(12*marker);
                } else {
                    channel7N[i]=channel7N[i]%12;
                }
            }
        } else if (channel==8) {
            guide=highest8;
            ...
            if(highest8-12*marker>0) {
                highest8-=(12*marker);
            } else {
                highest8=highest8%12;
            }
            if(lowest8-12*marker>0) {
                lowest8-=(12*marker);
            } else {
                lowest8=lowest8%12;
            }
            for (int i=0;i<channel8N.size();i++) {
                if(channel8N[i]-12*marker>0) {
                    channel8N[i]-=(12*marker);
                } else {
                    channel8N[i]=channel8N[i]%12;
                }
            }
        }  else if (channel==10) {
            guide=highest10;
            ...
            if(highest10-12*marker>0) {
                highest10-=(12*marker);
            } else {
                highest10=highest10%12;
            }
            if(lowest10-12*marker>0) {
                lowest10-=(12*marker);
            } else {
                lowest10=lowest10%12;
            }
            for (int i=0;i<channel10N.size();i++) {
                if(channel10N[i]-12*marker>0) {
                    channel10N[i]-=(12*marker);
                } else {
                    channel10N[i]=channel10N[i]%12;
                }
            }
        } else if (channel==11) {
            guide=highest11;
            ...
            if(highest11-12*marker>0) {
                highest11-=(12*marker);
            } else {
                highest11=highest11%12;
            }
            if(lowest11-12*marker>0) {
                lowest11-=(12*marker);
            } else {
                lowest11=lowest11%12;
            }
            for (int i=0;i<channel11N.size();i++) {
                if(channel11N[i]-12*marker>0) {
                    channel11N[i]-=(12*marker);
                } else {
                    channel11N[i]=channel11N[i]%12;
                }
            }
        } else if (channel==12) {
            guide=highest12;
            ...
            if(highest12-12*marker>0) {
                highest12-=(12*marker);
            } else {
                highest12=highest12%12;
            }
            if(lowest12-12*marker>0) {
                lowest12-=(12*marker);
            } else {
                lowest12=lowest12%12;
            }
            for (int i=0;i<channel12N.size();i++) {
                if(channel12N[i]-12*marker>0) {
                    channel12N[i]-=(12*marker);
                } else {
                    channel12N[i]=channel12N[i]%12;
                }
            }
        } else if (channel==13) {
            guide=highest13;
            ...
            if(highest13-12*marker>0) {
                highest13-=(12*marker);
            } else {
                highest13=highest13%12;
            }
            if(lowest13-12*marker>0) {
                lowest13-=(12*marker);
            } else {
                lowest13=lowest13%12;
            }
            for (int i=0;i<channel13N.size();i++) {
                if(channel13N[i]-12*marker>0) {
                    channel13N[i]-=(12*marker);
                } else {
                    channel13N[i]=channel13N[i]%12;
                }
            }
        } else if (channel==14) {
            guide=highest14;
            ...
            if(highest14-12*marker>0) {
                highest14-=(12*marker);
            } else {
                highest14=highest14%12;
            }
            if(lowest14-12*marker>0) {
                lowest14-=(12*marker);
            } else {
                lowest14=lowest14%12;
            }
            for (int i=0;i<channel14N.size();i++) {
                if(channel14N[i]-12*marker>0) {
                    channel14N[i]-=(12*marker);
                } else {
                    channel14N[i]=channel14N[i]%12;
                }
            }
        } else if (channel==15) {
            guide=highest15;
            ...
            if(highest15-12*marker>0) {
                highest15-=(12*marker);
            } else {
                highest15=highest15%12;
            }
            if(lowest15-12*marker>0) {
                lowest15-=(12*marker);
            } else {
                lowest15=lowest15%12;
            }
            for (int i=0;i<channel15N.size();i++) {
                if(channel15N[i]-12*marker>0) {
                    channel15N[i]-=(12*marker);
                } else {
                    channel15N[i]=channel15N[i]%12;
                }
            }
        }
    } else {
...
void meldMenu() {
    char answer;
    int meldnum;
    int tracka=16,trackb=16,trackc=16,trackd=16,tracke=16,trackf=16;
    string input;
    while (true) {
    ...
    while (true) {
        cout << "How many channels would you like to meld? (2-6)" << endl;
        getline(cin,input);
        stringstream myStream(input);
        if (myStream >> meldnum) {
            if ((meldnum>1)&(meldnum<7)) break;
        }
    }
    ...
    while (meldnum>3) {
        cout << "Choose the fourth channel to meld. (0-15)" << endl;
        getline(cin,input);
        stringstream myStream(input);
        if (myStream >> trackd) {
            if ((trackd>-1)&(trackd<16)&(trackd!=trackc)&(trackd!=trackb)&(trackd!=tracka)) break;
        }
    }

    while (meldnum>4) {
        cout << "Choose the fifth channel to meld. (0-15)" << endl;
        getline(cin,input);
        stringstream myStream(input);
        if (myStream >> tracke) {
            if ((tracke>-1)&(tracke<16)&(tracke!=trackd)&(tracke!=trackc)&(tracke!=trackb)&(tracke!=tracka)) break;
        }
    }

    while (meldnum>5) {
        cout << "Choose the sixth channel to meld. (0-15)" << endl;
        getline(cin,input);
        stringstream myStream(input);
        if (myStream >> trackf) {
            if ((trackf>-1)&(trackf<16)&(trackf!=trackd)&(trackf!=trackc)&(trackf!=trackb)&(trackf!=tracka)&(trackf!=tracke)) break;
        }
    }

    meld(tracka,trackb,trackc,trackd,tracke,trackf,meldnum);
}

void meld(int a, int b, int c,int d,int e,int f,int meldnum) {
    ...
    vector<short> trackNc;
    vector<unsigned long> trackTc;
    vector<short> trackNd;
    vector<unsigned long> trackTd;
    vector<short> trackNe;
    vector<unsigned long> trackTe;
    vector<short> trackNf;
    vector<unsigned long> trackTf;
    ...
    bool aDone=false,bDone=false,cDone=false,dDone=false,eDone=false,fDone=false;
    bool aClick=false,bClick=false,cClick=false,dClick=false,eClick=false,fClick=false;
    int j=0,k=0,l=0,m=0,n=0,o=0;
    int prevTime=0;
    unsigned long totala=trackTa[0];
    unsigned long totalb=trackTb[0];
    unsigned long totalc=-1;
    unsigned long totald=-1;
    unsigned long totale=-1;
    unsigned long totalf=-1;
    if (meldnum>2) {
        totalc=trackTc[0];
    }
    if (meldnum>3) {
    ...
    //cout << "got up to meld for loop..." << endl;

    for (int global=0;(!(aDone&bDone&cDone&dDone&eDone&fDone));global++) {
        aClick=false;
        bClick=false;
        if (meldnum<6) {
            fClick=true;
        ...
        while (!(aClick&bClick&cClick&dClick&eClick&fClick)) {
            ...
            if ((global==totalc)&(!cDone)&meldnum>2) {
                finalNotes.push_back(trackNc[l]);
                finalTimes.push_back(global-prevTime);
                prevTime=global;
                totalc+=trackTc[l+1];
                l++;
            ...
            if ((global==totald)&(!dDone)&meldnum>3) {
                finalNotes.push_back(trackNd[m]);
                finalTimes.push_back(global-prevTime);
                prevTime=global;
                totald+=trackTd[m+1];
                m++;
            ...
            if ((global==totale)&(!eDone)&meldnum>4) {
                finalNotes.push_back(trackNe[n]);
                finalTimes.push_back(global-prevTime);
                prevTime=global;
                totale+=trackTe[n+1];
                n++;
            ...
            if ((global==totalf)&(!fDone)&meldnum>5) {
                finalNotes.push_back(trackNf[o]);
                finalTimes.push_back(global-prevTime);
                prevTime=global;
                totalf+=trackTf[o+1];
                o++;
        ...
        if (l>=trackTc.size()|meldnum<3) cDone=true;
        if (m>=trackTd.size()|meldnum<4) dDone=true;
        if (n>=trackTe.size()|meldnum<5) eDone=true;
        if (o>=trackTf.size()|meldnum<6) fDone=true;
    }
...
void noter(int a) {
    ...
    //anti-redundant note call part

    for (int i=1;i<trackT.size();) {
        if ((trackT[i]<vol)&(trackT[i+3]<vol)&(trackT[i+2]<vol)&(trackT[i+1]<vol)&(trackT[i+4]<vol)&(trackT[i+5]<vol)) {
            if (trackN[i-1]%12==trackN[i]%12|
                trackN[i-1]%12==trackN[i+1]%12|
                trackN[i-1]%12==trackN[i+2]%12|
                trackN[i-1]%12==trackN[i+3]%12|
                trackN[i-1]%12==trackN[i+4]%12|
                trackN[i-1]%12==trackN[i+5]%12) {
                trackN.erase (trackN.begin()+(i-1));
                trackT.erase (trackT.begin()+i);
                continue;
            }
            if (trackN[i]%12==trackN[i+1]%12|
                trackN[i]%12==trackN[i+2]%12|
                trackN[i]%12==trackN[i+3]%12|
                trackN[i]%12==trackN[i+4]%12|
                trackN[i]%12==trackN[i+5]%12) {
                trackN.erase (trackN.begin()+i);
                trackT.erase (trackT.begin()+i+1);
                continue;
            }
            if (trackN[i+1]%12==trackN[i+2]%12|
                trackN[i+1]%12==trackN[i+3]%12|
                trackN[i+1]%12==trackN[i+4]%12|
                trackN[i+1]%12==trackN[i+5]%12) {
                trackN.erase (trackN.begin()+i+1);
                trackT.erase (trackT.begin()+i+2);
                continue;
            }
            if (trackN[i+2]%12==trackN[i+3]%12|
                trackN[i+2]%12==trackN[i+4]%12|
                trackN[i+2]%12==trackN[i+5]%12) {
                trackN.erase (trackN.begin()+i+2);
                trackT.erase (trackT.begin()+i+3);
                continue;
            }
            if (trackN[i+3]%12==trackN[i+4]%12|
                trackN[i+3]%12==trackN[i+5]%12) {
                trackN.erase (trackN.begin()+i+3);
                trackT.erase (trackT.begin()+i+4);
                continue;
            }
            if (trackN[i+4]%12==trackN[i+5]%12) {
                trackN.erase (trackN.begin()+i+4);
                trackT.erase (trackT.begin()+i+5);
                continue;
            }

            cout << "too many notes at once:" << endl;
            cout << notary[trackN[i-1]]<<","<<notary[trackN[i]]<<","<<notary[trackN[i+1]]<<","<<notary[trackN[i+2]]<<","<<notary[trackN[i+3]]<<","<<notary[trackN[i+4]]<<","<<notary[trackN[i+5]]<<endl;
            i+=7;
        }
        else if ((trackT[i]<vol)&(trackT[i+2]<vol)&(trackT[i+1]<vol)&(trackT[i+3]<vol)&(trackT[i+4]<vol)) {
            if (notary[trackN[i-1]]==notary[trackN[i]]|
                notary[trackN[i-1]]==notary[trackN[i+1]]|
                notary[trackN[i-1]]==notary[trackN[i+2]]|
                notary[trackN[i-1]]==notary[trackN[i+3]]|
                notary[trackN[i-1]]==notary[trackN[i+4]]) {
                trackN.erase (trackN.begin()+i-1);
                trackT.erase (trackT.begin()+i);
                continue;
            }
            if (notary[trackN[i]]==notary[trackN[i+1]]|
                notary[trackN[i]]==notary[trackN[i+2]]|
                notary[trackN[i]]==notary[trackN[i+3]]|
                notary[trackN[i]]==notary[trackN[i+4]]) {
                trackN.erase (trackN.begin()+i);
                trackT.erase (trackT.begin()+i+1);
                continue;
            }
            if (notary[trackN[i+1]]==notary[trackN[i+2]]|
                notary[trackN[i+1]]==notary[trackN[i+3]]|
                notary[trackN[i+1]]==notary[trackN[i+4]]) {
                trackN.erase (trackN.begin()+i+1);
                trackT.erase (trackT.begin()+i+2);
            ...
            if (notary[trackN[i+2]]==notary[trackN[i+3]]|
                notary[trackN[i+2]]==notary[trackN[i+4]]) {
                trackN.erase (trackN.begin()+i+2);
                trackT.erase (trackT.begin()+i+3);
                continue;
            }
            if (notary[trackN[i+3]]==notary[trackN[i+4]]) {
                trackN.erase (trackN.begin()+i+3);
                trackT.erase (trackT.begin()+i+4);
                continue;
            }
...
void keyChangeMenu(){
    ...
    if (answer=='N'|answer=='n') {
        ...
        while (true) {
            cout << "How many half-steps should it shift? Positive for up, negative for down (DO NOT INCLUDE +, ONLY -)." << endl;
            getline(cin,input);
            stringstream myStream(input);
            if (myStream >> keyShift) {
                if ((keyShift>-30)&(keyShift<30)) break;
            }
        }
    }
...
void octaveFolder() {
    for (int i=0;i<channel0N.size();i++) {
        while (channel0N[i]>36) {
            channel0N[i]-=12;
        }
        while (channel0N[i]<7) {
            channel0N[i]+=12;
        }
        if (channel0N[i]>newHighest0) {
            newHighest0=channel0N[i];
        }
        if (channel0N[i]<newLowest0) {
            newLowest0=channel0N[i];
        }
    }
    for (int i=0;i<channel1N.size();i++) {
        while (channel1N[i]>36) {
            channel1N[i]-=12;
        }
        while (channel1N[i]<7) {
            channel1N[i]+=12;
        }
        if (channel1N[i]>newHighest1) {
            newHighest1=channel1N[i];
        }
        if (channel1N[i]<newLowest1) {
            newLowest1=channel1N[i];
        }
    }
    for (int i=0;i<channel2N.size();i++) {
        while (channel2N[i]>36) {
            channel2N[i]-=12;
        }
        while (channel2N[i]<7) {
            channel2N[i]+=12;
        }
        if (channel2N[i]>newHighest2) {
            newHighest2=channel2N[i];
        }
        if (channel2N[i]<newLowest2) {
            newLowest2=channel2N[i];
        }
    }
    for (int i=0;i<channel3N.size();i++) {
        while (channel3N[i]>36) {
            channel3N[i]-=12;
        }
        while (channel3N[i]<7) {
            channel3N[i]+=12;
        }
        if (channel3N[i]>newHighest3) {
            newHighest3=channel3N[i];
        }
        if (channel3N[i]<newLowest3) {
            newLowest3=channel3N[i];
        }
    }
    for (int i=0;i<channel4N.size();i++) {
        while (channel4N[i]>36) {
            channel4N[i]-=12;
        }
        while (channel4N[i]<7) {
            channel4N[i]+=12;
        }
        if (channel4N[i]>newHighest4) {
            newHighest4=channel4N[i];
        }
        if (channel4N[i]<newLowest4) {
            newLowest4=channel4N[i];
        }
    }
    for (int i=0;i<channel5N.size();i++) {
        while (channel5N[i]>36) {
            channel5N[i]-=12;
        }
        while (channel5N[i]<7) {
            channel5N[i]+=12;
        }
        if (channel5N[i]>newHighest5) {
            newHighest5=channel5N[i];
        }
        if (channel5N[i]<newLowest5) {
            newLowest5=channel5N[i];
        }
    }
    for (int i=0;i<channel6N.size();i++) {
        while (channel6N[i]>36) {
            channel6N[i]-=12;
        }
        while (channel6N[i]<7) {
            channel6N[i]+=12;
        }
        if (channel6N[i]>newHighest6) {
            newHighest6=channel6N[i];
        }
        if (channel6N[i]<newLowest6) {
            newLowest6=channel6N[i];
        }
    }
    for (int i=0;i<channel7N.size();i++) {
        while (channel7N[i]>36) {
            channel7N[i]-=12;
        }
        while (channel7N[i]<7) {
            channel7N[i]+=12;
        }
        if (channel7N[i]>newHighest7) {
            newHighest7=channel7N[i];
        }
        if (channel7N[i]<newLowest7) {
            newLowest7=channel7N[i];
        }
    }
    for (int i=0;i<channel8N.size();i++) {
        while (channel8N[i]>36) {
            channel8N[i]-=12;
        }
        while (channel8N[i]<7) {
            channel8N[i]+=12;
        }
        if (channel8N[i]>newHighest8) {
            newHighest8=channel8N[i];
        }
        if (channel8N[i]<newLowest8) {
            newLowest8=channel8N[i];
        }
    }
    for (int i=0;i<channel10N.size();i++) {
        while (channel10N[i]>36) {
            channel10N[i]-=12;
        }
        while (channel10N[i]<7) {
            channel10N[i]+=12;
        }
        if (channel10N[i]>newHighest10) {
            newHighest10=channel10N[i];
        }
        if (channel10N[i]<newLowest10) {
            newLowest10=channel10N[i];
        }
    }
    for (int i=0;i<channel11N.size();i++) {
        while (channel11N[i]>36) {
            channel11N[i]-=12;
        }
        while (channel11N[i]<7) {
            channel11N[i]+=12;
        }
        if (channel11N[i]>newHighest11) {
            newHighest11=channel11N[i];
        }
        if (channel11N[i]<newLowest11) {
            newLowest11=channel11N[i];
        }
    }
    for (int i=0;i<channel12N.size();i++) {
        while (channel12N[i]>36) {
            channel12N[i]-=12;
        }
        while (channel12N[i]<7) {
            channel12N[i]+=12;
        }
        if (channel12N[i]>newHighest12) {
            newHighest12=channel12N[i];
        }
        if (channel12N[i]<newLowest12) {
            newLowest12=channel12N[i];
        }
    }
    for (int i=0;i<channel13N.size();i++) {
        while (channel13N[i]>36) {
            channel13N[i]-=12;
        }
        while (channel13N[i]<7) {
            channel13N[i]+=12;
        }
        if (channel13N[i]>newHighest13) {
            newHighest13=channel13N[i];
        }
        if (channel13N[i]<newLowest13) {
            newLowest13=channel13N[i];
        }
    }
    for (int i=0;i<channel14N.size();i++) {
        while (channel14N[i]>36) {
            channel14N[i]-=12;
        }
        while (channel14N[i]<7) {
            channel14N[i]+=12;
        }
        if (channel14N[i]>newHighest14) {
            newHighest14=channel14N[i];
        }
        if (channel14N[i]<newLowest14) {
            newLowest14=channel14N[i];
        }
    }
    for (int i=0;i<channel15N.size();i++) {
        while (channel15N[i]>36) {
            channel15N[i]-=12;
        }
        while (channel15N[i]<7) {
            channel15N[i]+=12;
        }
        if (channel15N[i]>newHighest15) {
            newHighest15=channel15N[i];
        }
        if (channel15N[i]<newLowest15) {
            newLowest15=channel15N[i];
        }
    }
        for (int i;i<finalNotes.size();i++) {
        while (finalNotes[i]>36) {
            finalNotes[i]-=12;
        }
        while (finalNotes[i]<7) {
            finalNotes[i]+=12;
        }
    }
}
